...
frontend/
  ...
  src/mapas.jsx   el globo de la portada y el mapa de ruta de la ficha (§9.4)
...
aeropuertos. Las usa el globo de la portada (§9.4) para saber dónde cae cada
aeropuerto y hacia dónde girar.
...
La web tiene **dos pantallas**: la **portada** (el globo y el buscador, §9.4) y
...
## 9.4 La portada: el globo terráqueo

Vive en `frontend/src/mapas.jsx` y es **SVG de verdad** (formas con contorno),
así que todos los colores salen de las variables `--mu-*` y se ve nítido en
cualquier pantalla.

- Al entrar **gira solo, despacio**. En escritorio ocupa toda la pantalla, con
  la marca y el buscador centrados encima; en móvil va **debajo** del buscador,
  para que el botón de buscar siga cayendo en la primera pantalla.
- Los **575 aeropuertos** salen marcados con un circulito. Van como un único
  `MultiPoint`, así que d3 esconde solo los de la cara oculta.
- **Se maneja con la mano**: arrastrar lo gira, la rueda y los botones `+ − ⟲`
  acercan hasta 8 aumentos, y **pulsar un aeropuerto lo elige como origen**. Se
  acierta el más cercano dentro de un radio medido **en píxeles de pantalla**
  (14 con ratón, 26 con el dedo), no en kilómetros: así la zona de acierto se
  siente igual con el globo grande y con el pequeño.
- Al elegir aeropuerto (por el globo o escribiéndolo) **deja de girar y viaja**
  hasta centrarlo. Al tocarlo también deja de girar, para no pelear con el
  usuario; el botón `⟲` recentra y lo devuelve a girar.
- **Desaparece en cuanto hay resultados.**

**Rendimiento, que es lo que condiciona el diseño.** El globo **no se repinta
con React**: el bucle escribe directamente el atributo `d` de tres rutas
(tierra, fronteras, aeropuertos), que es lo único que cambia. Gira a **24
imágenes por segundo**, no a 60, y se para solo cuando la pestaña no se ve o
cuando el sistema pide menos animaciones. Medido en navegador: 17–22 ms entre
fotogramas, en escritorio y a 390 px.
...
- **Globo y ficha del vuelo** comprobados en navegador real calculando por
  separado dónde debe caer cada aeropuerto en pantalla.
